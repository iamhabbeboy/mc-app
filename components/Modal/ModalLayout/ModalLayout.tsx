...
const Modal = ({ children, size, className }: Props) => {
  ...
  const fullContent = size === "full" ? "fixed h-screen" : "rounded-[40px] shadow-md p-10";

  return (
    <section className={`${defaultSize} ${mediumLayout} left-0 top-0  animate__animated ${className}  bg-black bg-opacity-50 w-screen h-screen ${fullContentSize}`}>
